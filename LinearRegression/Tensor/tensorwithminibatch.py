...
np.random.seed(0)
X = np.random.normal(distribution_params[1]['mean'],
                     distribution_params[1]['std'],
                     size=(n_sample, feature_dim))
...
theta = tf.Variable(np.random.normal(0, 1, size=feature_dim + 1), dtype=tf.float32)
...
for epoch in range(epochs):
    for batch_X, batch_y in get_batches(X, y, batch_size):
        batch_tf_X = tf.constant(batch_X, dtype=tf.float32)
        batch_tf_y = tf.constant(batch_y, dtype=tf.float32)
        with tf.GradientTape() as tape:
            predictions = tf.matmul(batch_tf_X, tf.reshape(theta, (-1, 1)))[:, 0]
            loss = compute_loss(batch_tf_y, predictions)
        grads = tape.gradient(loss, [theta])
        optimizer.apply_gradients(zip(grads, [theta]))

    # Record parameters and loss at the end of each epoch
